...
public class ProductDao implements IGeneralProductDao {
    ...
    public Product findById(int id) throws SQLException, ClassNotFoundException {
        Product product;
        connection = DBConnection.getConnection();
        PreparedStatement preparedStatement = connection.prepareStatement("select * from product join category on product.id = cateogry.id where id = ?");
        ...
        ResultSet resultSet = preparedStatement.executeQuery();
            ...
            String description = resultSet.getString(5);
            String category = resultSet.getString(8);
            product = new Product(name, price,color,description,category);
